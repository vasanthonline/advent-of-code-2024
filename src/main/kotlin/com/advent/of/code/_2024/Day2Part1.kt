package com.advent.of.code._2024
...
class Day2Part1 {
    ...
    fun run(input: String) {
        val inputs = input.split("\n").map{ it.split("\\s".toRegex()).map { ip -> ip.toInt() } }
        val answer = inputs.fold(0) { acc, ip ->
            val isIncreasing = ip[0] > ip[1]
            val isSafe = ip.foldIndexed(true) { index, accInner: Boolean, inp ->
                when(index) {
                    0 -> accInner
                    else -> {
                        val diff = abs(ip[index-1] - inp)
                        accInner &&
                                (diff in 1..3 && (
                                        (isIncreasing && ip[index-1] > inp) ||
                                                (!isIncreasing && ip[index-1] < inp)
                                        )
                                )
                    }
